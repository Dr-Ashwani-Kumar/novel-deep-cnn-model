...
class BaseHHO(Optimizer):
    ...
    def evolve(self, epoch):
        ...
        for idx in range(0, self.pop_size):
            ...
            _, x_p, worst = self.get_special_solutions(self.pop, best=1, worst=1)  # updated
            g_best = x_p[0], g_worst = worst[0]  # udpated
            if np.abs(E) >= 1:
                # Harris' hawks perch randomly based on 2 strategy:
                if np.random.rand() >= 0.5:  # perch based on other family members
                    des = (epoch + 1) / (np.random.uniform() * self.epoch + self.EPSILON)   # updated from sparrow
                    if des > 5:
                        des = np.random.normal()  # updated from sparrow
                    ...
                    pos_new = 0.5*((self.pop[idx][self.ID_POS] * np.exp(des))+X_rand - np.random.uniform() * np.abs(
                        X_rand - 2 * np.random.uniform() * self.pop[idx][self.ID_POS]))  # updated from sparrow
                    ...
                    pos_new = 0.5*(self.pop[idx][self.ID_POS] + np.random.normal() * np.ones(self.problem.n_dims)+(self.g_best[self.ID_POS] - X_m) - np.random.uniform() * \
                              (self.problem.lb + np.random.uniform() * (self.problem.ub - self.problem.lb)))  # updated from sparrow
            ...
            # -------- Exploitation phase -------------------
                ...
                if (np.random.rand() >= 0.5):
                    ...
                    if np.abs(E) >= 0.5:  # Hard besiege Eq. (6) in paper
                        ...
                        pos_new = a1*(np.random.normal() * np.exp((g_worst[self.ID_POS] - self.pop[idx][self.ID_POS]) / (idx + 1) ** 2))+a2*(self.g_best[self.ID_POS] - E * np.abs(delta_X))+\
                                  a3*(E * np.abs(delta_X) -self.pop[idx][self.ID_POS])  # updated
                    pos_new = self.amend_position(pos_new, self.problem.lb, self.problem.ub)
                    pop_new.append([pos_new, None])
                else:
                    ...
                    if np.abs(E) >= 0.5:  # Soft besiege Eq. (10) in paper
                        ...
                        # Y = self.g_best[self.ID_POS] - E * np.abs(
                        #     J * self.g_best[self.ID_POS] - self.pop[idx][self.ID_POS])
                        delta_X = self.g_best[self.ID_POS] - self.pop[idx][self.ID_POS]
                        Y = 0.5*(self.g_best[self.ID_POS] + E * np.abs(
                            J * self.g_best[self.ID_POS] - self.pop[idx][self.ID_POS])+self.g_best[self.ID_POS] + beta*(E * np.abs(delta_X) -self.pop[idx][self.ID_POS]))
                    else:  # Hard besiege Eq. (11) in paper
                        X_m = np.mean([x[self.ID_POS] for x in self.pop])
                        # Y = self.g_best[self.ID_POS] - E * np.abs(J * self.g_best[self.ID_POS] - X_m)
                        Y = 0.5*(self.g_best[self.ID_POS] - E * np.abs(J * self.g_best[self.ID_POS] - X_m) + self.g_best[self.ID_POS]
                            + (g_worst[self.ID_POS] - self.pop[idx][self.ID_POS]))
